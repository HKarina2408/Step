...
// Завдання:
// Помістити у Map (schedule) дані з scheduleObj
// new Date(): [{}...]
...
class FlightBS {
  ...
  static getAllPlanesForDate = (date) => {
    // all the planes for mentioned date of []
    return FlightBS.schedule.get(flightDates[date]) || [];
  };

  static getAllSittingPlacesForDate = (date) => {
    // Завдання:
    // Зважаючи на date порахувати загальну кількість сидячих місць на день
    // capacity
...
console.log(planesForDate);

const planesWrongForDate = FlightBS.getAllPlanesForDate("2025-09-11");
console.log(planesWrongForDate);

const capacityForDate = FlightBS.getAllSittingPlacesForDate("2024-06-29");
console.log(capacityForDate);
...
console.log(FlightBS.addSchedulePlanes("2024-06-30", morePlanes));

console.log(FlightBS.addSchedulePlanes("2024-06-29", morePlanes));
...
console.log(defaultMap.has("key"));
...
// Напишіть функцію, яка приймає строку - речення і повертає Map,
// де ключами є слова, а значеннями - кількість їх повторень у цьому реченні.

const wordsCount = (sentense) => {
...
const words = wordsCount("something text something");
console.log(words);
