...
class A {
  ...
  constructor(a) {
    ...
    if (A.instance) {
      return A.instance;
      //   throw new Error(
      //     "You are allowed to create only one instance of the class"
      //   );
    }
...
class PostAPI {
  ...
  constructor(url, options) {
    ...
    if (PostAPI.instance) return PostAPI.instance;
  }
...
proxyObject.name = "Karina";
proxyObject.city = "Lviv";
...
const afterAdvancedCustomer = advancedCustomer.buy(1000, 10);
console.log(afterAdvancedCustomer);
...
observable.subscribe(registerUser);
observable.subscribe(activateUser);
...
const ecryprPasswordMiddleware = (data) => {
  data.password = data.password.replaceAll("1", "$");

  return data;
};

const addNameMiddlevare = (data) => {
  data.name = "Karina";
...
deleteUser(userToDelete, deletePost, descryptPassword, deleteName);
